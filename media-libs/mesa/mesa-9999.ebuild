...
if [[ ${PV} == 9999 ]]; then
	...
	SRC_URI="
		https://crates.io/api/v1/crates/proc-macro2/1.0.56/download -> proc-macro2-1.0.56.tar.gz
		https://crates.io/api/v1/crates/quote/1.0.25/download -> quote-1.0.25.tar.gz
		https://crates.io/api/v1/crates/syn/2.0.15/download -> syn-2.0.15.tar.gz
		https://crates.io/api/v1/crates/unicode-ident/1.0.6/download -> unicode-ident-1.0.6.tar.gz
	"
	inherit git-r3
else
...
pkg_pretend() {
	...
	if use video_cards_nouveau && use vulkan; then
		ewarn "Experimental nouveau vulkan (NVK) support has been enabled."
	fi
}
...
src_prepare() {
	...
	tar -xzf "${DISTDIR}/proc-macro2-1.0.56.tar.gz" -C "${S}/subprojects/"
	tar -xzf "${DISTDIR}/quote-1.0.25.tar.gz" -C "${S}/subprojects/"
	tar -xzf "${DISTDIR}/syn-2.0.15.tar.gz" -C "${S}/subprojects/"
	tar -xzf "${DISTDIR}/unicode-ident-1.0.6.tar.gz" -C "${S}/subprojects/"

	cp "${FILESDIR}/proc-macro2-1.0.56-meson.build" "${S}/subprojects/proc-macro2-1.0.56/meson.build"
	cp "${FILESDIR}/quote-1.0.25-meson.build" "${S}/subprojects/quote-1.0.25/meson.build"
	cp "${FILESDIR}/syn-2.0.15-meson.build" "${S}/subprojects/syn-2.0.15/meson.build"
	cp "${FILESDIR}/unicode-ident-1.0.6-meson.build" "${S}/subprojects/unicode-ident-1.0.6/meson.build"
}
